...
dependencies {
    ...
    paperweight.paperDevBundle("1.21.4-R0.1-SNAPSHOT") // contains 1.18.2 nms classes
    ...
    if ( project.hasProperty("1.16") == true ) {
        ...
        tasks.named<RemapJar>("reobfJar") {
            ...
            dependsOn(tasks.jar)
            base {
                archivesName.set("${OUTPUT_JAR_NAME}-${target}-SNAPSHOT")
            }
        }
    }
        ...
        // apply<PaperweightUser>() // applies the paper weight plugin for minecraft nms classes
        paperweight.paperDevBundle("1.19.4-R0.1-SNAPSHOT") // contains 1.18.2 nms classes
        ...
        // apply<PaperweightUser>() // applies the paper weight plugin for minecraft nms classes
        paperweight.paperDevBundle("1.20-R0.1-SNAPSHOT") // contains 1.18.2 nms classes
        ...
        sourceSets["main"].java.srcDir("src/nms/v1_21_R3")
        paperweight.paperDevBundle("1.21.4-R0.1-SNAPSHOT")
        target = "1.21.4"

        // no reobf linkage for 1.21.4; build uses shadowJar output
    }
}
...
tasks {
    named<ShadowJar>("shadowJar") {
        ...
        doFirst {
            val supportedMinecraftVersions = setOf("1.16.5", "1.18.2", "1.19.4", "1.20", "1.21.4")
            if ( !supportedMinecraftVersions.contains(target) ) {
                throw Exception("Invalid Minecraft version! Supported versions are: 1.16.5, 1.18.2, 1.19.4, 1.20, 1.21.4")
            }
        }
...
gradle.taskGraph.whenReady {
    tasks {
        named<ShadowJar>("shadowJar") {
            if ( hasTask(":release") ) {
                archiveBaseName.set("${OUTPUT_JAR_NAME}-${target}")
