...
public data class Gun(
    ...
    public val projectileParticleMinDistance: Double = 4.0,
    ...
    public val soundShoot: String = "minecraft:item.crossbow.shoot",
    public val soundShootVolume: Float = 6f,
    public val soundShootPitch: Float = 1f,
    public val soundEmpty: String = "minecraft:block.note_block.hat",
    public val soundEmptyVolume: Float = 1f,
    public val soundEmptyPitch: Float = 1f,
    public val soundReloadStart: String = "minecraft:item.crossbow.loading_start",
    public val soundReloadStartVolume: Float = 1f,
    public val soundReloadStartPitch: Float = 1f,
    public val soundReloadFinish: String = "minecraft:item.crossbow.loading_end",
                ...
                toml.getTable("projectile.particles")?.let { particles ->
                    val particleType = particles.getString("type")?.let { ty ->
                        try {
                            ...
                            err.printStackTrace()
                            // DUST replaces legacy REDSTONE in newer APIs; fallback is safe
                            Particle.DUST
                        }
                    } ?: Particle.DUST
                    properties["projectileParticleType"] = particleType
                    ...
                    particles.getDouble("min_distance")?.let { properties["projectileParticleMinDistance"] = it }
                ...
                toml.getTable("sound")?.let { sound ->
                    if ( sound.isTable("shoot") ) {
                        sound.getTable("shoot")?.let { s ->
                            s.getString("name")?.let { name ->
                                properties["soundShoot"] = if (name.contains(":")) name else "minecraft:item.crossbow.shoot"
                            }
                            s.getDouble("volume")?.let { properties["soundShootVolume"] = it.toFloat() }
                            s.getDouble("pitch")?.let { properties["soundShootPitch"] = it.toFloat() }
                        }
                    } else {
                        sound.getString("shoot")?.let { name ->
                            properties["soundShoot"] = if (name.contains(":")) name else "minecraft:item.crossbow.shoot"
                        }
                    }
                    ...
                    if ( sound.isTable("reload_start") ) {
                        sound.getTable("reload_start")?.let { s ->
                            s.getString("name")?.let { name ->
                                properties["soundReloadStart"] = if (name.contains(":")) name else "minecraft:item.crossbow.loading_start"
                            }
                        ...
                        sound.getString("reload_start")?.let { name ->
                            properties["soundReloadStart"] = if (name.contains(":")) name else "minecraft:item.crossbow.loading_start"
                        }
                    }
                    ...
                    if ( sound.isTable("reload_finish") ) {
                        sound.getTable("reload_finish")?.let { s ->
                            s.getString("name")?.let { name ->
                                properties["soundReloadFinish"] = if (name.contains(":")) name else "minecraft:item.crossbow.loading_end"
                            }
                        ...
                        sound.getString("reload_finish")?.let { name ->
                            properties["soundReloadFinish"] = if (name.contains(":")) name else "minecraft:item.crossbow.loading_end"
                        }
                    }
                    ...
                    if ( sound.isTable("impact") ) {
                        sound.getTable("impact")?.let { s ->
                            s.getString("name")?.let { name ->
                                properties["soundImpact"] = if (name.contains(":")) name else "minecraft:entity.generic.explode"
                            }
                        ...
                        sound.getString("impact")?.let { name ->
                            properties["soundImpact"] = if (name.contains(":")) name else "minecraft:entity.generic.explode"
                        }
                    }
                    ...
                    if ( sound.isTable("explosion") ) {
                        sound.getTable("explosion")?.let { s ->
                            s.getString("name")?.let { name ->
                                properties["soundExplosion"] = if (name.contains(":")) name else "minecraft:entity.generic.explode"
                            }
                        ...
                        sound.getString("explosion")?.let { name ->
                            properties["soundExplosion"] = if (name.contains(":")) name else "minecraft:entity.generic.explode"
                        }
                    }
                }
