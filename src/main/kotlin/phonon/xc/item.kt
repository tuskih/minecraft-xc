...
import phonon.xc.nms.getTag
import phonon.xc.nms.setTag
import phonon.xc.nms.hasTag
import phonon.xc.XC
...
public fun XC.getGunInHand(player: Player): Gun? {
    ...
    if ( nmsItem != null ) {
        getGunFromNmsItemStack(nmsItem)?.let { return it }
    }

    val bukkitItem = craftPlayer.getInventory().getItemInMainHand()
    if ( bukkitItem != null ) {
        return this.getGunFromItem(bukkitItem)
    }
...
public fun XC.getGunInHandUnchecked(player: Player): Gun? {
    ...
    if ( nmsItem != null ) {
        getCustomItemUnchecked(nmsItem, this.storage.gun)?.let { return it }
    }

    val bukkitItem = craftPlayer.getInventory().getItemInMainHand()
    if ( bukkitItem != null ) {
        val itemMeta = bukkitItem.getItemMeta()
...
public fun XC.getGunFromItem(item: ItemStack): Gun? {
    if ( item.type == this.config.materialGun ) {
        try {
            getGunFromItemNMS(item)?.let { return it }
        } catch (err: Exception) {
