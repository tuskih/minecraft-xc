...
public class EventListener(
    ...
    @EventHandler
    public fun onWorldLoad(e: WorldLoadEvent) {
        xc.addWorldSystems(e.world)
        if ( xc.doDebugTimings ) {
            xc.logger.info("Added world systems for ${e.world.name}")
        }
    }

    @EventHandler
    public fun onWorldUnload(e: WorldUnloadEvent) {
        xc.removeWorldSystems(e.world)
        if ( xc.doDebugTimings ) {
            xc.logger.info("Removed world systems for ${e.world.name}")
        }
    }
    ...
    public fun onInteract(e: PlayerInteractEvent) {
        ...
        if ( xc.doDebugTimings ) {
            val t = xc.getItemTypeInHand(player)
            xc.logger.info("onInteract action=$action itemType=$t")
        }
        ...
        if ( action == Action.LEFT_CLICK_AIR || action == Action.LEFT_CLICK_BLOCK ) {
            when ( xc.getItemTypeInHand(player) ) {
                ...
                XC.ITEM_TYPE_GUN -> {
                    if ( xc.doDebugTimings ) {
                        val mid = xc.getCustomItemIdInHand(player, XC.ITEM_TYPE_GUN)
                        val loaded = if ( mid >= 0 && mid < xc.storage.gun.size ) (xc.storage.gun[mid] != null) else false
                        xc.logger.info("Gun left-click: modelId=$mid loaded=$loaded")
                    }
                    xc.getGunInHandUnchecked(player)?.let { _ ->
                        ...
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Queued playerShootRequest for ${player.name}")
                        }
                    } ?: run {
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Gun not mapped in storage, skipping queue for ${player.name}")
                        }
                    }
                }
                ...
                XC.ITEM_TYPE_THROWABLE -> {
                    xc.getThrowableInHandUnchecked(player)?.let {
                        ...
                        e.setUseInteractedBlock(Event.Result.DENY)
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Queued readyThrowableRequest for ${player.name}")
                        }
                    }
                }
            }
            ...
            when ( xc.getItemTypeInHand(player) ) {
                ...
                XC.ITEM_TYPE_GUN -> {
                    if ( xc.doDebugTimings ) {
                        val mid = xc.getCustomItemIdInHand(player, XC.ITEM_TYPE_GUN)
                        val loaded = if ( mid >= 0 && mid < xc.storage.gun.size ) (xc.storage.gun[mid] != null) else false
                        xc.logger.info("Gun right-click: modelId=$mid loaded=$loaded")
                    }
                    xc.getGunInHandUnchecked(player)?.let { gun ->
                        ...
                        if ( gun.autoFire ) {
                            ...
                            e.setUseInteractedBlock(Event.Result.DENY)
                            if ( xc.doDebugTimings ) {
                                xc.logger.info("Queued playerAutoFireRequest for ${player.name}")
                            }
                        }
                    } ?: run {
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Gun not mapped in storage, skipping auto-fire queue for ${player.name}")
                        }
                    }
                }
                ...
                XC.ITEM_TYPE_THROWABLE -> {
                    xc.getThrowableInHandUnchecked(player)?.let {
                        ...
                        e.setUseInteractedBlock(Event.Result.DENY)
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Queued throwThrowableRequest for ${player.name}")
                        }
                    }
                }
                ...
                XC.ITEM_TYPE_HAT -> {
                    xc.getHatInHandUnchecked(player)?.let {
                        ...
                        e.setUseInteractedBlock(Event.Result.DENY)
                        if ( xc.doDebugTimings ) {
                            xc.logger.info("Queued wearHatRequest for ${player.name}")
                        }
                    }
                }
            }
