...
public class XC(
    ...
    internal fun initializeSystems() {
        ...
        newCalculatePlayerSpeedTask.runTaskTimerAsynchronously(plugin, 0L, 1L)
        ...
        if ( config.antiCombatLogEnabled ) {
            ...
            newAntiCombatLogTask.runTaskTimerAsynchronously(plugin, 0L, 1L)
            antiCombatLogTask = newAntiCombatLogTask
        }
    }

    internal fun addWorldSystems(world: World) {
        val worldId = world.getUID()
        if ( projectileSystems[worldId] == null ) {
            projectileSystems[worldId] = ProjectileSystem(world, this.threadpool, this.threadpoolSize)
        }
        if ( thrownThrowables[worldId] == null ) {
            thrownThrowables[worldId] = ArrayList()
        }
        if ( expiredThrowables[worldId] == null ) {
            expiredThrowables[worldId] = ArrayList()
        }
        if ( landmineExplosions[worldId] == null ) {
            landmineExplosions[worldId] = ArrayList()
        }
    }

    internal fun removeWorldSystems(world: World) {
        val worldId = world.getUID()
        projectileSystems.remove(worldId)
        thrownThrowables.remove(worldId)
        expiredThrowables.remove(worldId)
        landmineExplosions.remove(worldId)
    }
    ...
    internal fun update() {
        ...
        if ( config.asyncPackets ) {
            ...
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
                TaskSpawnParticleBulletTrails(plugin, particleBulletTrails),
            )
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
                TaskSpawnParticleBulletBlockImpacts(plugin, particleBulletBlockImpacts),
            )
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
                TaskSpawnParticleBulletHitboxImpacts(plugin, particleBulletHitboxImpacts),
            )
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
                TaskSpawnParticleExplosion(plugin, particleExplosions),
            )
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
            ...
            Bukkit.getScheduler().runTaskAsynchronously(
                plugin,
                TaskSounds(plugin, soundPackets),
            )
        }
        else {
            // sync
            ...
            TaskSpawnParticleBulletTrails(plugin, particleBulletTrails).run()
            TaskSpawnParticleBulletBlockImpacts(plugin, particleBulletBlockImpacts).run()
            TaskSpawnParticleBulletHitboxImpacts(plugin, particleBulletHitboxImpacts).run()
            TaskSpawnParticleExplosion(plugin, particleExplosions).run()
            TaskAmmoInfoMessages(gunAmmoInfoMessages).run()
            TaskSounds(plugin, soundPackets).run()
        }
